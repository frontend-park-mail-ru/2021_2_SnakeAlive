...
import {CreateReview, DataType, dispatcher, EventType, ID, Token} from '@/dispatcher';
...
export default class ReviewReducer {
    ...
    #sendDeleteReview = (reviewID: number): Promise<Response> =>
        sendDeleteJSONRequest(backendEndpoint + reviewsURI + reviewID)
            .then(response => {
                if (response.status === 404) {
                    return Promise.reject(new Error('На сайте нет такой страницы'));
                }
